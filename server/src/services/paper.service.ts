...
export const enqueuePdfGeneration = async (paperId: string): Promise<{ jobId: string }> => {
  ...
  const job = await pdfQueue.add(
    'generate-pdf',
    { paperId },
    { attempts: 3, backoff: { type: 'exponential', delay: 1500 }, removeOnComplete: 20, removeOnFail: 50 }
  );
