...
const parseQuestionTypes = (rawValue) => {
...
export const createAssignmentController = asyncHandler(async (request, response) => {
...
export const listAssignmentsController = asyncHandler(async (request, response) => {
...
export const getAssignmentController = asyncHandler(async (request, response) => {
    const id = Array.isArray(request.params.id) ? request.params.id[0] : request.params.id;
    const assignment = await getAssignmentById(id ?? '');
    response.json({ data: assignment });
});
export const generateAssignmentController = asyncHandler(async (request, response) => {
    const id = Array.isArray(request.params.id) ? request.params.id[0] : request.params.id;
